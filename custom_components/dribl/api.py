...
class DriblApi:
    ...
    async def resolve_tenant(self, slug: str) -> dict:
        ...
        return {
            "id": tenant["id"],
            "name": tenant["attributes"].get("tenantable_name")
            or tenant["attributes"]["name"],
            "slug": slug,
        }
    ...
    async def get_leagues(
        self, tenant_id: str, competition_id: str, club_id: str
    ) -> list[dict]:
        ...
        # Return all leagues for the competition; the user picks their specific team
        return [
            {"id": league["id"], "name": league["attributes"]["name"]}
